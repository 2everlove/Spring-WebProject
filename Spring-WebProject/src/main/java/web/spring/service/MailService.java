...
public class MailService {		
	...
	public void welcomeMailSend() {
		 ...
		 * prop.put("mail.smtp.ssl.trust", "smtp.gmail.com"); prop.put("mail.smtp.host",
		 * "smtp.gmail.com");// SMTP서버 prop.put("mail.smtp.auth", "true");// SMTP 서버의 인증
		 * 사용 prop.put("mail.smtp.port", "587");// TLS 포트번호= 587, SSL 포트번호= 465
		 */
		...
		try {
			...
			// 발송자 설정 (발송자의 메일, 발송자명)
			msg.setFrom(new InternetAddress("dreamingskywhale@gmail.com", "모비딕"));
             // 수신자 설정 
			...
			// 메일 내용
			msg.setText("ID는 nasajang2020@gmail.com 입니다.", "UTF-8");
			msg.setText("mailtest", "UTF-8");
	...
	public void sendMailPwd(String email, String encodingPwd) {
		...
		try {
			...
			msg.setSentDate(new Date());
			// 발송자 설정 (발송자의 메일, 발송자명)
			msg.setFrom(new InternetAddress("dreamingskywhale@gmail.com", "webadmin"));
             // 수신자 설정 
			// Message.RecipientType.TO : 받는 사람 
	...
	public boolean sendPwd(String pwd, String email) {
		...
		try {
			...
			msg.setFrom(new InternetAddress("dreamingskywhale@gmail.com", "모비딕"));
             // 수신자 설정 
			// Message.RecipientType.TO : 받는 사람 
			InternetAddress to = new InternetAddress("dreamingskywhale@gmail.com");
			msg.setRecipient(Message.RecipientType.TO, to);
			
            // 메일 제목
			msg.setSubject("비밀번호 재설정 안내", "UTF-8");
			// 메일 내용
