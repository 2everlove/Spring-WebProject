			...
			success : function(data, status, xhr) {
				...
				$.each(data.list, function(index, item) {
					...
					htmlContent += "<div class='u_cbox_comment_box u_cbox_type_profile'>"
								...
								+ item.user_id
								+ "<c:if test='${sessionScope.user.user_type eq 0 || sessionScope.user.user_id eq" + item.user_id + "}'>"
								+ "<button type='button' onclick='deleteAjax("+ item.review_num +")' class='trash'> <i class='fas fa-trash'></i> </button>"
								+ "</c:if>"
								+ "</div>" 
...
<c:choose>
	<c:when test="${empty sessionScope.user.user_id}">
		<div class="comment_section_login">
			<div class="login_text"><p>리뷰를 작성하시려면 로그인 해주세요</p>
			<div><button class="login_needed" onclick="location.href='/login'"><p style="color:#a29bfe; font-weight: bold;" >로그인 하기</p></button></div>
			</div>
		</div>
	</c:when>
	<c:otherwise>
	<div class="comment_section">
		<div class="top-wrap">
			<input type="hidden" id="user_id" name ="user_id" class="comment_id" value="${sessionScope.user.user_id}"><div id="output_userid"><c:out value="${sessionScope.user.user_id}"></c:out> </div>
			<p class="product_rating">
				<div class="jq-stars star stardiv"></div>
				<input type="hidden" id="review_rate" name="review_rate" class="product_rating" value="">
				<input type="hidden" name="pboard_unit_no" id="pboard_unit_no" value="${pBoard.pboard_unit_no}">
			<p>
		</div>
		<div class="middle-wrap">
			<div class="input_area">
				<textarea rows="10" class="text" id="text" name="review_content" placeholder="댓글을 입력해주세요" wrap="hard"></textarea>
			</div>
		</div>
